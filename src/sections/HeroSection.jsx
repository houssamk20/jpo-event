...
const HeroSection = () => {
  ...
  return (
    <section id="overview" className="relative min-h-screen flex items-center overflow-hidden pt-16">
      {/* Video Background - Optimized for all screens */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        {/* Desktop Layout (lg and above) - Scaled left with medium-large size */}
        <div className="hidden lg:block">
          <div className="max-w-2xl -ml-32">
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:gap-4 ml-20 mt-6">
              <Link 
                to="/workshops"
                className="bg-cyan-400 hover:bg-cyan-300 text-black px-10 py-5 md:px-14 md:py-6 font-black text-xl md:text-2xl transition-colors uppercase tracking-wider text-center rounded-lg inline-block"
              >
                Register Now
              </Link>
            </div> 
        ...
        {/* Mobile & Tablet Layout (below lg) - Responsive version */}
        <div className="lg:hidden">
          <div className="flex flex-col items-center justify-center text-center w-full px-4">
            ...
            <div className="w-full max-w-[240px] xs:max-w-[280px] sm:max-w-[320px] ml-4 xs:ml-6 sm:ml-8">
              <Link 
                to="/workshops"
                className="w-full bg-cyan-400 hover:bg-cyan-300 active:bg-cyan-500 text-black px-7 py-4 xs:px-8 xs:py-4.5 sm:px-10 sm:py-5 font-black text-lg xs:text-xl sm:text-2xl transition-all duration-200 uppercase tracking-wider transform hover:scale-105 active:scale-95 rounded-lg inline-block text-center"
              >
                Register Now
              </Link>
            </div> 
      ...
      {/* Fixed Back to Overview Arrow Button - MEDIUM SIZE */}
      <button
        onClick={scrollToOverview}
        className="fixed right-5 xs:right-6 sm:right-8 md:right-10 bottom-5 xs:bottom-6 sm:bottom-8 md:bottom-10 z-50 bg-cyan-400 hover:bg-cyan-300 text-black p-3 xs:p-3.5 sm:p-4 rounded-full shadow-xl hover:shadow-cyan-400/50 transition-all duration-200 transform hover:scale-110 active:scale-95 group"
